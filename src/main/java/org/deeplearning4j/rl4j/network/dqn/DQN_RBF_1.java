...
public class DQN_RBF_1<NN extends DQN_RBF> implements IDQN<NN> {
    ...
    public void setNeuralNetworks(NeuralNetwork[] new_mln) {
        this.mln = (MultiLayerNetwork) new_mln[0];
    }

    public static DQN_RBF load(String path) throws IOException {
        return new DQN_RBF(ModelSerializer.restoreMultiLayerNetwork(path));
    }
    ...
    public NN clone() {
        NN nn = (NN)new DQN_RBF(mln.clone());
