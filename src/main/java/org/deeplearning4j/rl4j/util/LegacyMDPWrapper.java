...
public class LegacyMDPWrapper<OBSERVATION extends Encodable, A, AS extends ActionSpace<A>> implements MDP<Observation, A, AS> {
    ...
    public LegacyMDPWrapper(EnvConstructive<OBSERVATION, A, AS> wrappedMDP, IHistoryProcessor historyProcessor) {
    ...
    private void createTransformProcess() {
        ...
        if(historyProcessor != null && shape.length == 3) {
            ...
            transformProcess = TransformProcess.builder()
                    ...
                    .transform("data", (Operation) new MultiImageTransform(
    ...
    public StepReply<Observation> step(A a) {
        ...
        INDArray rawObservation = getInput((EnvState) rawStepReply.getObservation());
    ...
    private void record(OBSERVATION obs) {
        INDArray rawObservation = getInput((EnvState) obs);
    ...
    private INDArray getInput(EnvState obs) {
        Observation obs_i = obs.getObservation();
        return obs_i.getData();
    }
...
*/    
    ...
    public void setState(Observation obs){
        wrappedMDP.setState(obs);
    }
    
    public void setReward(double obs){
        wrappedMDP.setReward(obs);
    }
/*    @Override
    public ObservationSpace<Observation> getObservationSpace() {
        return (ObservationSpace<Observation>) wrappedMDP.getObservationSpace();
    }
*/
